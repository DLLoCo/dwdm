...
def build_dataloaders(config: dict):
    ...
    if data_dir and os.path.exists(train_path):
        ...
        if result is not None:
            ...
            # Compute approximate time indices for each sample
            # NYCTaxi: 2015-01-01 (Thursday=3) to 2015-03-01, 48 steps/day
            # 3-day lookback offset ≈ 144 steps
            def compute_time_indices(n_samples, offset):
                indices = np.arange(n_samples) + offset
                hour_slot = indices % 48                       # 0-47
                day_of_week = ((indices // 48) + 3) % 7        # Thu=3 for Jan 1
                return hour_slot.astype(np.int64), day_of_week.astype(np.int64)

            offset0 = 144  # 3 days * 48 steps
            h_train, d_train = compute_time_indices(len(x_train), offset0)
            h_val, d_val = compute_time_indices(len(x_val), offset0 + len(x_train))
            h_test, d_test = compute_time_indices(len(x_test), offset0 + len(x_train) + len(x_val))
            print(f"  Time indices: hour_slot [0-47], day_of_week [0-6]")

            def make_loader(x, y, hour, dow, shuffle):
            ...
            return (make_loader(x_train, y_train, h_train, d_train, True),
                    make_loader(x_val, y_val, h_val, d_val, False),
                    make_loader(x_test, y_test, h_test, d_test, False),
                    scaler, adj)
        else:
    ...
    loaders = []
    offset = 0
    for d_raw, shuf in zip([data[:t1], data[t1:t2], data[t2:]], [True, False, False]):
